...
def check_neighbours(tile, end, came_from, open, closed, discovered, mapa):
    ...
    for vizinho, custo_terreno in vizinhos:
        if vizinho in closed or vizinho in discovered:
            continue

        # Na busca gulosa, somente a heurística h define a prioridade.
        # O custo do terreno não participa da decisão.
        h = abs(end_x - vizinho[0]) + abs(end_y - vizinho[1])
        discovered.add(vizinho)
        came_from[vizinho] = tile
        heapq.heappush(open, (h, vizinho))
...
def find_path(start, end, mapa):
    ...
    while len(open) > 0:
        ...
        closed.add(atual)
        check_neighbours(atual, end, came_from, open, closed, discovered, mapa)
